...
class VideoToSingleChannel:
    """Convert RGB mask videos to single-channel by taking channel 0.

    Used in inpainting datasets where the mask is loaded as RGB (default for
    LoadVideo) but should be (T, 1, H, W) for downstream consumers.

    Operates on sample dicts whose values are tensors (e.g. from the cached
    .pth path in UnifiedDataset).  Keys whose names start with "mask" and
    whose value is a 4-D tensor with shape[1] == 3 are stripped to 1 channel.
    Values that are not tensors (e.g. lists of PIL Images from the live path)
    are passed through unchanged.
    """
    def __call__(self, sample):
        sample = sample.copy() if isinstance(sample, dict) else sample
        for key in list(sample.keys()) if isinstance(sample, dict) else []:
            v = sample[key]
            if key.startswith("mask") and hasattr(v, "ndim") and v.ndim == 4 and v.shape[1] == 3:
                sample[key] = v[:, :1]
        return sample
