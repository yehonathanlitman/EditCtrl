...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        _has_inpaint_global = (
            getattr(self, "enable_inpaint_global", False)
            and getattr(self, "gp_seq_len", 0) > 0
        )
        if self.has_image_input:
            img = y[:, :257]
            ctx = y[:, 257:]
        elif _has_inpaint_global:
            ctx = y[:, :-self.gp_seq_len]
            econd = y[:, -self.gp_seq_len:]
        else:
            ctx = y
        q_proj = self.q(x)
        q = self.norm_q(q_proj)
        ...
        if self.has_image_input:
            ...
            q_e = self.editctrl_norm_q(q_proj)
            k_e = self.editctrl_norm_k(self.editctrl_k(econd))
            v_e = self.editctrl_v(econd)
            x = x + self.editctrl_zero_proj(self.attn(q_e, k_e, v_e))
        return self.o(x)
