...
class WanVideoPipeline(BasePipeline):
    ...
    def from_pretrained(
        ...
        # --- Inpainting global path (inference) ---------------------
        if enable_inpaint_global and pipe.dit is not None:
            import copy
            from ..models.wan_video_dit import RMSNorm as _InpaintRMSNorm

            def _attach_inpaint_layers(dit_module):
                dit_module.editctrl_patch_embedding = copy.deepcopy(dit_module.patch_embedding)
                dit_module.enable_inpaint_global = True
                for block in dit_module.blocks:
                    ca = block.cross_attn
                    if not ca.has_image_input and not hasattr(ca, "enable_inpaint_global"):
                        ca.editctrl_k = torch.nn.Linear(ca.dim, ca.dim).to(device=pipe.device, dtype=pipe.torch_dtype)
                        ca.editctrl_v = torch.nn.Linear(ca.dim, ca.dim).to(device=pipe.device, dtype=pipe.torch_dtype)
                        ca.editctrl_norm_k = _InpaintRMSNorm(ca.dim, eps=1e-6).to(device=pipe.device, dtype=pipe.torch_dtype)
                        ca.editctrl_norm_q = _InpaintRMSNorm(ca.dim, eps=1e-6).to(device=pipe.device, dtype=pipe.torch_dtype)
                        ca.editctrl_zero_proj = torch.nn.Linear(ca.dim, ca.dim, bias=True).to(device=pipe.device, dtype=pipe.torch_dtype)
                        torch.nn.init.zeros_(ca.editctrl_zero_proj.weight)
                        torch.nn.init.zeros_(ca.editctrl_zero_proj.bias)
                        ca.enable_inpaint_global = True
                        ca.gp_seq_len = 0

            _attach_inpaint_layers(pipe.dit)
            if getattr(pipe, "dit2", None) is not None:
                _attach_inpaint_layers(pipe.dit2)
        # ---------------------------------------------------------------------
    ...
    def __call__(
        ...
        inpaint_latent_surgery: bool = False,
        # progress_bar
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # Scheduler
            latents_step = self.scheduler.step(noise_pred, self.scheduler.timesteps[progress_id], inputs_shared["latents"])
            if inpaint_local_enabled and inputs_shared.get("mask_bool_latent") is not None:
                _mb = inputs_shared["mask_bool_latent"]
                _L = inputs_shared["latents"]
                _T, _H, _W = _L.shape[2], _L.shape[3], _L.shape[4]
                _m = _mb.view(1, 1, _T, _H, _W).expand_as(_L).to(_L.device)
                inputs_shared["latents"] = torch.where(_m, latents_step, _L)
            else:
                inputs_shared["latents"] = latents_step
        ...
        if inpaint_latent_surgery and inputs_shared.get("mask_bool_latent") is not None and inputs_shared.get("input_latents") is not None:
            ...
            orig = inputs_shared["input_latents"].to(latents.device, dtype=latents.dtype)
            inputs_shared["latents"] = torch.where(mask, latents, orig)
...
class WanVideoUnit_InpaintMask(PipelineUnit):
    ...
    downsampled input latents used by the global path.
    ...
    def process(
        ...
        out = {
            ...
            "mask_bool_token_dilate": mask_token.flatten().to(torch.bool),
...
def model_fn_wan_video(
    ...
    mask_bool_token_dilate: torch.Tensor = None,
    ...
    **kwargs,
    ...
    if inpaint_global_enabled and downsampled_input_latents is not None and hasattr(dit, "editctrl_patch_embedding"):
        gp = dit.editctrl_patch_embedding(downsampled_input_latents)   # (B, dim, f, h, w)
        gp = gp.flatten(2).transpose(1, 2)                            # (B, f*h*w, dim)
        _gp_seq = gp.shape[1]
        for _blk in dit.blocks:
            if getattr(_blk.cross_attn, "enable_inpaint_global", False):
                _blk.cross_attn.gp_seq_len = _gp_seq
    ...
    _dit_mask = mask_bool_token_dilate if mask_bool_token_dilate is not None else mask_bool_token
    _inpaint_local_active = (
        inpaint_local_enabled
        and _dit_mask is not None
        and not use_unified_sequence_parallel
    )
    if _inpaint_local_active:
        _full_x = x
        x = x[:, _dit_mask, :]
        _full_freqs = freqs
        freqs = freqs[_dit_mask]
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if vace_context is not None and block_id in vace.vace_layers_mapping:
                ...
                if _inpaint_local_active:
                    current_vace_hint = current_vace_hint[:, _dit_mask, :]
